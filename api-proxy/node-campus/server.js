...
function buildSystemPrompt(stats) {
  ...
  return [
    ...
    "完整真相：上一轮人类已经接近星际时代，但尚未真正殖民外星。零点能湮灭炸弹是地表试验场中的试制品，瞄准中央地壳靶井，实际当量约为预期一百万倍，造成太平洋盆地级联塌陷、长期地质崩毁和后世大洪水传说。战争因灭世洪水被迫停止。随后欧罗巴一号在地震与火山喷发中毁灭，钛合金一体铸造护板挡不住沸腾岩浆，离心机爆炸将尚未完成的反基因病毒送入大气；人类大规模死亡，幸存者逐渐丧失繁殖能力，生物圈进入灾难。",
...
function localMinervaReply(message, stats = {}) {
  ...
  if (text.includes("零点") || text.includes("湮灭") || text.includes("太平洋") || text.includes("马里亚纳") || text.includes("大洪水")) {
    return "零点能湮灭武器不是基因武器。它是在地表试验场瞄准中央地壳靶井的试制品，不是太空炮击，也不是海底打击；失控后一百万倍放大，才把地质灾难推向全球。";
  }
...
function shouldUseLocalCanon(message) {
  return /欧罗巴|离心机|钛合金|岩浆|零点|湮灭|太平洋|马里亚纳|大洪水|诺亚|朊|病毒|蛋白|外星|殖民|星海|帝国|宇宙/.test(String(message || ""));
}
...
async function handleChat(req, res) {
  ...
  try {
    ...
    if (shouldUseLocalCanon(message)) {
      send(res, 200, JSON.stringify({ reply: localMinervaReply(message, fallbackStats), canon: true }));
      return;
    }
